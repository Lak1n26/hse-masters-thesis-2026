...
class MLPv12(NBRBase):
    def __init__(self, train_baskets, test_baskets,valid_baskets,dataset ,basket_count_min=0, min_item_count = 5, user_embed_size = 32,item_embed_size = 128,h1 = 128,h2 = 128,h3 = 128,h4 = 128,h5 = 128,history_len = 40, job_id = 1):
        ...
        # Use both input3 (recency) and input4 (intervals) as features
        x3_reshaped = Reshape((self.history_len,1))(input3)
        x4_reshaped = Reshape((self.history_len,1))(input4)
        
        # Concatenate all features: user/item embedding repeated + recency + intervals
        x14 = Concatenate()([x12, x3_reshaped, x4_reshaped])
        x14 = Dense(h1, activation= 'relu')(x14)

        # LSTM layers process the temporal features
        x = LSTM(h2, return_sequences=True)(x14)
        x = LSTM(h3)(x)
    ...
    def create_train_data(self):
        ...
        for c,user in enumerate(self.test_users):
            ...
            for i in range(max(0,len(baskets)-5), len(baskets)):  # берем не последние 50 корзин, а 10
    ...
    def train(self):
        ...
        # Use verbose=1 for standard Keras progress bar
        history = self.model.fit(
            [train_items,train_users,train_history,train_history2],
            train_labels, 
